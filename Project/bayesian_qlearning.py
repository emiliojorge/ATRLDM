...
class Bayesian_Qlearning():
    ...
    def VPI(self, state):
        ...
        # print("c", c)
        vpi = []
        for action, (a, mu, b, l, c_i) in enumerate(
                zip(self.alpha[state, :], self.mu0[state, :], self.beta[state, :], self.lam[state, :], c)):
            if action == a_star:
    ...
    def posterior(self, state, action, M1, M2, update=True):
        ...
        beta_new = self.beta[state, action] + self.n[state, action] / 2 * (M2 - M1 ** 2) + (
                self.n[state, action] * self.lam[state, action] * (M1 - self.mu0[state, action]) ** 2 / (
                2 * (self.lam[state, action] + self.n[state, action]))
        )
        if update == True:
    ...
    def update_mixed(self, state, action, reward, next_state):
        def integrand(x, mu, tau):
            ...
            E_r_sq = (self.lam[next_state, next_action] + 1) / self.lam[next_state, next_action] * (
                    self.beta[next_state, next_action] / (self.alpha[next_state, next_action] - 1)) + self.mu0[
        ...
        new_alpha = np.max(1 + 0.001,
                           brentq(lambda y: np.log(y) - digamma(y) - (np.log(E_tau) - E_log_tau), 0.0001, 10 ** 8))
    ...
    def observe_transition(self, state, action, next_state, reward):
        ...
        """
        td_error = reward + self.discount * np.max(self.mu0[next_state, ...]) - self.mu0[state, action]
